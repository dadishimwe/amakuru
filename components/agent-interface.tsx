...
const ALL_PRS = [
  { id: 145, title: "story: district road blockage update",        agent: "verifier",   status: "review",   comments: 2, additions: 57, deletions: 4,  branch: "tips/road-blockage",      time: "Just now" },
  { id: 144, title: "update: source credibility methodology",      agent: "auditor",    status: "review",   comments: 1, additions: 18, deletions: 3,  branch: "scoring/methodology-v1",  time: "1m ago" },
  { id: 143, title: "publish: audio summary (Kinyarwanda)",        agent: "producer",   status: "merged",   comments: 4, additions: 93, deletions: 11, branch: "audio/daily-brief",       time: "1m ago" },
  { id: 142, title: "verify: power outage reports by sector",      agent: "monitor",    status: "merged",   comments: 3, additions: 84, deletions: 12, branch: "signals/power-outage",    time: "2m ago" },
  { id: 141, title: "approve: correction + provenance trail",      agent: "editor",     status: "approved", comments: 1, additions: 31, deletions: 8,  branch: "corrections/provenance", time: "8m ago" },
  { id: 140, title: "trend: what's trending in Kigali (today)",    agent: "curator",    status: "review",   comments: 5, additions: 142,deletions: 27, branch: "trends/kigali-today",     time: "22m ago" },
  { id: 139, title: "data: cost-of-living index snapshot",         agent: "analyst",    status: "merged",   comments: 7, additions: 209,deletions: 88, branch: "data/cost-index",         time: "1h ago" },
]

const ALL_REVIEW_FILES = [
  { file: "verification/tip.schema.ts", pct: 72 },
  { file: "scoring/methodology.json",   pct: 45 },
  { file: "signals/trending.ts",        pct: 88 },
  { file: "audio/summarize.ts",         pct: 31 },
  { file: "stories/provenance.ts",      pct: 60 },
]

const ALL_REVIEW_LINES: { type: "comment"|"approve"|"change"|"code"; text: string; author?: string }[] = [
  { type: "code",    text: 'const tier = verify.locationTier("district")' },
  { type: "comment", text: "Add evidence requirement for high-impact claims", author: "auditor" },
  { type: "code",    text: 'score.attachCitations(story.sources)' },
  { type: "approve", text: "LGTM — scoring rationale is clear", author: "editor" },
  { type: "code",    text: 'export const summarizeAudio = async (story) => {' },
  { type: "change",  text: "Add Kinyarwanda-first fallback here", author: "translator" },
  { type: "code",    text: '  return publishPodcast(brief)' },
  { type: "approve", text: "Approved — publish it", author: "producer" },
  { type: "code",    text: 'provenance.append({ change, timestamp })' },
  { type: "comment", text: "Show what changed in the UI", author: "curator" },
  { type: "code",    text: 'return { status: "verified", story }' },
  { type: "approve", text: "All checks pass", author: "verifier" },
]

const COMMITS = [
  { hash: "a3f8c21", msg: "feat: provenance trail for corrections", time: "Just now" },
  { hash: "b7d2e09", msg: "feat: audio daily brief (rw/en/fr)",     time: "4m ago"   },
  { hash: "c9a1f34", msg: "chore: update scoring methodology v1",   time: "12m ago"  },
  { hash: "d4e6b78", msg: "perf: faster trending by time window",   time: "31m ago"  },
  { hash: "e2c9d56", msg: "feat: expertise badges for commenters",  time: "1h ago"   },
]
    ...
    >
      <div style={{
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr 1fr", borderBottom: "1px solid rgba(0,0,0,0.06)", background: "rgba(251,250,247,0.9)" }}>
          {[
            { label: "Reports verified",     val: 18,       icon: <CheckCircle2 style={{ width: 11, height: 11 }} />, graph: <MiniBarGraph seed={0} /> },
            { label: "Sources scored",       val: 34,       icon: <Eye style={{ width: 11, height: 11 }} />,          graph: <MiniBarGraph seed={5} /> },
            { label: "Corrections tracked",  val: 127,      icon: <GitCommit style={{ width: 11, height: 11 }} />,    graph: <MiniDotGraph seed={2} /> },
            { label: "Signals / min",        val: reqCount, icon: <Zap style={{ width: 11, height: 11 }} />,          graph: <LiveSparkline seed={7} /> },
          ].map((m, i) => (
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1.1fr 1fr 0.85fr", gap: 8, padding: 8, height: 340, overflow: "hidden" }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 5, height: "100%", overflow: "hidden", ...anim(160) }}>
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "0 2px", flexShrink: 0 }}>
              ...
              <span style={{ fontSize: 8.5, letterSpacing: "0.13em", textTransform: "uppercase", color: "rgba(0,0,0,0.38)", fontFamily: "monospace" }}>Story Queue</span>
              </div>
            <span style={{ fontSize: 7.5, color: "rgba(0,0,0,0.25)", fontFamily: "monospace" }}>{ALL_PRS.filter(p => p.status === "review").length} NEEDS REVIEW</span>
            </div>
            ...
            <div style={{ ...panel, padding: "8px 10px", flexShrink: 0, height: 76, overflow: "hidden" }}>
              <div style={{ display: "flex", alignItems: "center", gap: 5, marginBottom: 6 }}>
                ...
                <span style={{ fontSize: 7.5, letterSpacing: "0.13em", textTransform: "uppercase", color: "rgba(0,0,0,0.33)", fontFamily: "monospace" }}>Verification Activity</span>
              </div>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 5, height: "100%", overflow: "hidden", ...anim(210) }}>
            <div style={{ display: "flex", alignItems: "center", gap: 5, padding: "0 2px", flexShrink: 0 }}>
              ...
              <span style={{ fontSize: 8.5, letterSpacing: "0.13em", textTransform: "uppercase", color: "rgba(0,0,0,0.38)", fontFamily: "monospace" }}>Verification Review — #{ALL_PRS[reviewFileIdx % ALL_PRS.length].id}</span>
            </div>
            <div style={{ ...panel, flex: 1, padding: "9px 10px", overflow: "hidden", display: "flex", flexDirection: "column" }}>
              ...
              <div style={{ marginBottom: 7, paddingBottom: 7, borderBottom: "1px solid rgba(0,0,0,0.05)", flexShrink: 0 }}>
                <div style={{ fontSize: 9.5, fontWeight: 600, color: "#111", marginBottom: 2 }}>verify: evidence + tiers for community tips</div>
                <div style={{ display: "flex", gap: 5 }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 5, height: "100%", overflow: "hidden", ...anim(260) }}>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 5, padding: "2px 2px 0", flexShrink: 0 }}>
              ...
              <span style={{ fontSize: 8.5, letterSpacing: "0.13em", textTransform: "uppercase", color: "rgba(0,0,0,0.38)", fontFamily: "monospace" }}>Pipelines</span>
            </div>
            <div style={{ ...panel, flexShrink: 0, overflow: "hidden" }}>
              {[
                { name: "curation",      status: "passing", duration: "1m 32s" },
                { name: "verification",  status: "running", duration: "0m 48s" },
                { name: "audio-brief",   status: "passing", duration: "2m 11s" },
                { name: "trending",      status: "running", duration: "0m 54s" },
              ].map((a, i) => (
